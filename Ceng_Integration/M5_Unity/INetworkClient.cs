/// <summary>
/// File: INetworkClient.cs
/// Brief: WebSocket Network Interface for Unity to communicate with Raspberry Pi 5
/// Author: Ziya 210104004027
/// Date: 2026-03-27
/// Version: 0.2 (Ceng_Integration)
///
/// Changelog:
/// v0.1 - Initial draft, defined connection and audio streaming events.
/// v0.2 (Ceng_Integration) - Docx compliance revision:
///   [R11] SendOperatorCommand(string) → SendOperatorCommand(CommandPacket) (docx §2B)
/// </summary>

using System;

public interface INetworkClient
{
    /// <summary>
    /// Event triggered when a new telemetry JSON is received and parsed.
    /// </summary>
    event Action<TelemetryData> OnTelemetryReceived;

    /// <summary>
    /// Connects to the Raspberry Pi WebSocket server.
    /// </summary>
    /// <param name="ipAddress">The IP address of the Pi 5 (e.g., "ws://192.168.1.10:5000")</param>
    void Connect(string ipAddress);

    /// <summary>
    /// Disconnects gracefully from the server.
    /// </summary>
    void Disconnect();

    /// <summary>
    /// [R11] Sends a manual override command to the robot.
    /// Original: was SendOperatorCommand(string command).
    /// Reason for change: Docx §2B defines "Drive JSON:
    ///   {"override": true, "cmd": "FORWARD"}".
    /// Without the override flag, M4 cannot distinguish whether
    /// the incoming command is manual or autonomous.
    /// CommandPacket class was added in DataContracts.cs via [R10].
    /// </summary>
    /// <param name="cmd">Override flag + drive command (CommandPacket)</param>
    void SendOperatorCommand(CommandPacket cmd);

    /// <summary>
    /// Sends the recorded microphone audio blob for Edge STT processing.
    /// </summary>
    /// <param name="wavData">Byte array of the .wav file</param>
    void SendAudioBlob(byte[] wavData);
}