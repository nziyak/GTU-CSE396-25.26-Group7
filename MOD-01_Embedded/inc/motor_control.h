...
/**
 * @brief  Initialise motor control from a hardware config struct.
 * @param  motor   Pointer to uninitialised MotorControl_t instance.
 * @param  config  Pointer to populated MotorConfig_t.
 * @return HAL_OK on success.
 */
HAL_StatusTypeDef Motor_Init(MotorControl_t *motor, const MotorConfig_t *config);

/**
 * @brief  Execute a movement command (FORWARD, BACKWARD, LEFT, RIGHT, STOP).
 * @param  motor    Motor control instance.
 * @param  command  MotorCommand_t value.
 * @return HAL_OK on success.
 */
...
/**
 * @brief  Active brake — sets all IN pins HIGH, stops immediately.
 * @param  motor  Motor control instance.
 */
void Motor_EmergencyStop(MotorControl_t *motor);

/**
 * @brief  Returns current motor status.
 */
MotorStatus_t Motor_GetStatus(const MotorControl_t *motor);

/**
 * @brief  Returns true if motors are currently driving (not stopped).
 */
bool Motor_IsActive(const MotorControl_t *motor);

/**
 * @brief  Returns the last executed command.
 */
